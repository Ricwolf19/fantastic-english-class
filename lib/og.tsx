...
/** Acento de color por sección — coherente con la paleta comic del sitio. */
const OG_ACCENTS = {
  pink: "#ff2e97",
  lime: "#8be042",
  sky: "#35c9f0",
...
type OgOptions = {
  ...
  eyebrow: string;
  description?: string;
  /** Chips de features (máx 3 se ven bien). Vacío = no se renderiza la fila. */
  tags?: string[];
  /** Pie derecho (p. ej. "Presencial u online"). */
  footerRight?: string;
  /** Pie izquierdo. Default: ubicación. */
  footerLeft?: string;
  accent?: OgAccent;
};

/**
 * Tarjeta Open Graph 1200×630 con estética dark comic: fondo azul-noche, glow
 * y halftone del color de acento, mascota, wordmark, eyebrow, título grande,
 * descripción, chips de features y pie con ubicación/modalidad. El acento varía
 * por sección para que cada imagen se distinga de un vistazo.
 */
export const renderOg = ({
  ...
  description = "",
  tags = [],
  footerRight = "Presencial u online",
  footerLeft = "Chihuahua, México",
  accent = "pink",
}: OgOptions) => {
  const color = OG_ACCENTS[accent];
  ...
  return new ImageResponse(
    <div
      style={{
        ...
        padding: "68px 76px",
        background: "#120f23",
        backgroundImage: `radial-gradient(1100px 520px at 88% -18%, ${color}38, transparent), radial-gradient(760px 460px at -8% 120%, #8be04222, transparent)`,
      }}
    >
      {/* Barra superior de acento (viñeta comic) */}
      <div
        style={{
          position: "absolute",
          top: 0,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "18px" }}>
        ...
        >
          <span style={{ color }}>Fantastic</span>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
        ...
        >
          <span style={{ width: "40px", height: "5px", background: color }} />
          {eyebrow}
        </div>
        <div
          style={{
